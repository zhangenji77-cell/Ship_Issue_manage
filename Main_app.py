...
# --- 3. 登录界面 (包含缩小后的 Logo) ---
def login_ui():
    # ✅ 调整列比例为 [2, 1, 2]，使中间放置 Logo 的列变窄，从而实现 Logo 缩小
    ...
    with col_logo:
        try:
            ...
            st.warning("⚠️ 未找到 TSM_Logo.png，请检查文件路径。")

    st.markdown("<h2 style='text-align: center;'>Trust Ship 系统登录</h2>", unsafe_allow_html=True)
    st.markdown(
        "<p style='text-align: center; color: gray;'>注：为保障 50 艘船的数据安全，本系统不保存登录状态，刷新页面需重新登录。</p>",
        unsafe_allow_html=True)
...
st.sidebar.write(f"当前权限: `{st.session_state.role}`")
        ...
        with col_l:
            ...
            if not h_df.empty:
                ...
                if st.session_state.confirm_del_id:
                    st.warning(f"⚠️ 确定删除 (ID: {st.session_state.confirm_del_id})？")
                    b1, b2 = st.columns(2)
                    with b1:
...
if st.session_state.role == 'admin':
    with tabs[1]:
        ...
        if not m_df.empty:
            ...
            if to_del and st.button("执行物理删除"):
                with get_engine().begin() as conn:
